...
class PandasExecutor(Executor):
        ...
        if measure_attr != "":
            ...
            if isFiltered or has_color and attr_unique_vals:
                ...
                if len(result_vals) != N_unique_vals * color_cardinality:
                    ...
                    if has_color:
                        df = pd.DataFrame({columns[0]: list(attr_unique_vals) * color_cardinality,columns[1]: pd.Series(color_attr_vals).repeat(N_unique_vals),})
    ...
    def compute_stats(self, ldf: LuxDataFrame):
        ...
        for attribute in ldf.columns:
            ...
            # ====== begin Prototype ======
            if ldf._histograms:
                ldf.unique_values[attribute_repr] = list(ldf._histograms[attribute].keys())
